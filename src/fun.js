const dareMessage = dares[Math.floor(Math.random() * dares.length)];
    ...
    react: "😈" // Bold, mischievous vibe for dares
},
{
    command: ['fact'],
    ...
    react: "📚" // Book for knowledge/facts
},
{
    command: ['jokes', 'joke'],
...
},
{
    ...
    react: "🤓" // Nerdy, fun vibe for memes
},
{
    command: ['quotes'],
    operate: async ({ m, reply }) => {
    ...
    react: "💭" // Thought bubble for inspirational quotes
},
{
    command: ['trivia'],
    operate: async ({ Cypher, m, reply }) => {
    ...
    react: "❓" // Question mark for trivia
},
{
    command: ["truth"],
    ...
    react: "🕵️" // Detective for revealing truths
},
{
    ...
    react: "🔍" // Magnifying glass for lie detection
},
{
    command: ['xxqc'],
    ...
    react: "🎨" // Paint palette for colorful quote stickers
}